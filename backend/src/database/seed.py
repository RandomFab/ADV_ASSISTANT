...
def generate_produits() -> list[Produit]:
    ...
    produits_data = [
        ...
        (
            "COIL-ALU-1.0",
            "Coil Aluminium 1050 ép.1mm",
            FamilleEnum.coil,
            MatiereEnum.aluminium,
            "1050",
            1.0,
            None,
            1900,
...
def generate_commandes(
    clients: list[Client], produits: list[Produit], n: int = 600
) -> list[Commande]:
    commandes = []
    ...
    for i in range(n):
        ...
        if age_jours > 60:
            ...
            statut = random.choices(
                [
                    StatutCommandeEnum.livree,
                    StatutCommandeEnum.expediee,
                    StatutCommandeEnum.en_production,
                ],
                weights=[60, 25, 15],
            )[0]
        else:
        ...
        if statut == StatutCommandeEnum.livree:
            retard = random.randint(
                -3, 10
            )  # Parfois en avance, souvent à l'heure, parfois en retard
        ...
        for produit in produits_choisis:
            ...
            prix = produit.prix_unitaire_eur * random.uniform(
                0.95, 1.05
            )  # Légère variation de prix
...
def generate_reclamations(
    clients: list[Client], commandes: list[Commande], n: int = 60
) -> list[Reclamation]:
    # On ne prend que les commandes livrées ou expédiées pour les réclamations
    commandes_eligibles = [
        c
        for c in commandes
        if c.statut in (StatutCommandeEnum.livree, StatutCommandeEnum.expediee)
    ]
