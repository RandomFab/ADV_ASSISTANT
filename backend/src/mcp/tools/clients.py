...
def get_client_info(client_name: str) -> dict:
    ...
    try:
        client = (
            db.query(Client)
            .options(
                joinedload(Client.commandes).joinedload(Commande.reclamations)
            )
        ...
        ca_cumule = sum(cmd.montant_total_eur if cmd.statut.value == "livree" else 0 for cmd in client.commandes)
        commandes_info = []
        for cmd in sorted(client.commandes, key=lambda c: c.date_commande, reverse=True)[:5]:
