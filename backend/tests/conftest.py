...
def mock_session_local(db_session, monkeypatch):
    ...
    monkeypatch.setattr("src.database.connection.SessionLocal", mock_sessionlocal)
    monkeypatch.setattr("src.mcp.tools.clients.SessionLocal", mock_sessionlocal)
    monkeypatch.setattr("src.mcp.tools.orders.SessionLocal", mock_sessionlocal)
    monkeypatch.setattr("src.mcp.tools.delivery.SessionLocal", mock_sessionlocal)
    monkeypatch.setattr("src.mcp.tools.stock.SessionLocal", mock_sessionlocal)
    monkeypatch.setattr("src.mcp.tools.reclamations.SessionLocal", mock_sessionlocal)
...
def tmp_interactions_log(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.config.paths.INTERACTIONS_LOG", log_file)
    monkeypatch.setattr("src.monitoring.logger.INTERACTIONS_LOG", log_file)
...
def mcp_test_env(mock_session_local, db_with_seed):
    ...
    return {"db_session": db_with_seed, "session_local": mock_session_local}


@pytest.fixture
def monitoring_test_env(
    tmp_interactions_log, sample_interactions_log, mock_github_env, mock_github_requests
):
    """Combined fixture for monitoring testing."""
    return {"log_file": sample_interactions_log, "github_post": mock_github_requests}
